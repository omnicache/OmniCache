...
{	
    [Collection("Tests")]
    public class LessThanEqualTests : BaseTest
    {
        public LessThanEqualTests(ITestOutputHelper output) : base(output)
        {
        ...
        {
            List<StoreStock> stocks = await cachedDB.GetMultipleAsync(query1, 21);
        ...
        {
            List<StoreStock> stocks = await cachedDB.GetMultipleAsync(query2, 21);
